...
class MonteCarloGraph:
    ...
    _delta = 0.75 # for visible area distance score (for hidden nodes)
    _theta = 5.0 # for shadow distance score (for visible nodes)
    _beta = 0.5 # for guard distance score
    _gamma = 1.0 # for kernel distance score
    _tau = 0.1 # for distance-decay rate
    _top_k = 5 # how many kernels to path plan to in scoring
    _eta = 5.0   # depth bonus weight

    _lambda = 4.0 # weight of rollout value in heuristic
    _phi = 7 # rollout depth
    _rollout_ratio = False # True if using survived steps/rollout_depths for score, False is using 0 or 1

    _visible_node_penalty = -100 # Score given to positions visible by a guard [0, -5, -10, -100]
    
    _brute_force = False # using brute force moves

    def __init__(self, map: Map):
        self._map = map
        if (not self._map.is_valid_position(self._map.get_player_start_pos())):
            raise RuntimeError(f"Start position {self._map.get_player_start_pos()} is not valid.")
        print(f"Got player start pos {self._map.get_player_start_pos()}")
        
        self._nodes = []
        self._transposition_table = {}
        self._tt_queries = 0
        self._tt_hits = 0
        self._root = MonteCarloGraph.Node(self, map.get_player_start_pos(), 0)
    ...
    def select(self) -> Tuple[MonteCarloGraph.Node, List[MonteCarloGraph.Edge]]:
        node = self._root
        path_edges = []

        while True:            
            moves = node.get_potential_moves()            
            allowed_children = min(len(moves), max(1, math.floor(MonteCarloGraph._k * (node._visits ** MonteCarloGraph._alpha))))
            can_expand = len(moves) > 0 and not node._fully_expanded and len(node._outgoing_edges) < allowed_children            
            # If the node is a leaf, or needs to be expanded, end selection
            if len(moves) == 0 or len(node._outgoing_edges) == 0 or can_expand:
                return node, path_edges
            
            # Otherwise, continue selection by choosing best edge
            best_edge = max(node._outgoing_edges, key=lambda e: e.ucb_score())
            path_edges.append(best_edge)
            node = best_edge._child
    
    def expand(self, node : MonteCarloGraph.Node) -> Optional[Tuple[MonteCarloGraph.Node, MonteCarloGraph.Edge]]:
        ...
        def ensure_cached(target):
            if target in node._moves:
                return node._moves[target]
            res = self._map.get_longest_move_along_shortest_path(node._loc, target)
            if res is None:
                node._moves[target] = None
                return None
            path, new_loc = res
            if len(path) == 1:
                path, new_loc = [node._loc, new_loc], new_loc

            visible = self._map.is_visible(new_loc, node._depth + 1)
            node._moves[target] = (path, new_loc, visible)
            return node._moves[target]

        # do one pass, looking for a hidden move
        for target in moves:
            cached = ensure_cached(target)
            if cached is None:
                continue
            path, new_loc, visible = cached
            
            # Skip this move if it exists
            if any(self._map.is_same_position(new_loc, uloc) for uloc in used_locs):
                continue

            # Expand move immediately if it's hidden
            if not visible and self._map.is_valid_position(new_loc):
                child = self._get_or_create_node(new_loc, node._depth + 1)
                edge = MonteCarloGraph.Edge(node, child, LineString(path))
                node._outgoing_edges.append(edge)
                child._incoming_edges.append(edge)
                #print(f"Adding move at depth {node._depth} towards {target} by going to {new_loc}")
                return child, edge
        
        # No more unexpanded hidden moves exist, expand first visible move
        for target in moves:
            cached = ensure_cached(target)
            if cached is None:
                continue
            ...
            if any(self._map.is_same_position(new_loc, uloc) for uloc in used_locs):
                continue
            if self._map.is_valid_position(new_loc):
                child = self._get_or_create_node(new_loc, node._depth + 1)
                edge = MonteCarloGraph.Edge(node, child, LineString(path))
                node._outgoing_edges.append(edge)
                child._incoming_edges.append(edge)
        ...
        # If no possible hidden or visible moves, node is fully expanded
        node._fully_expanded = True
        return None  # no new moves
        
    def rollout(self, node: MonteCarloGraph.Node):
        cur_loc = node._loc
        cur_depth = node._depth
        survived_steps = 0
        last_hidden = False

        for _ in range(MonteCarloGraph._phi):
        ...
        if MonteCarloGraph._rollout_ratio:
            return survived_steps / MonteCarloGraph._phi 
        # otherwise return 1 if last state hidden, else 0
        return 1.0 if last_hidden else 0.0

    def simulate(self, node : MonteCarloGraph.Node):
        return node.get_score()
    
    def backpropagate(self, leaf : MonteCarloGraph.Node, path_edges : List[MonteCarloGraph.Edge], result: float):
    ...
    def run(self, total_time):
        ...
        while True:
            ...
            if result is not None:
                ...
                edges.append(edge)
                value = self.simulate(new_node)
            else:
                # Expand called on fully expanded node
                value = leaf.get_score()
    ...
    def _get_or_create_node(self, loc, depth, store=True):
        ...
        node = MonteCarloGraph.Node(self, loc, depth)
    ...
    def get_stats(self):
        ...
        stats += f"c={MonteCarloGraph._c} (exploration constant)\n"
        stats += f"k={MonteCarloGraph._k} (progressive widening constant)\n"
        stats += f"alpha={MonteCarloGraph._alpha} (progressive widening exponent)\n"
        stats += f"beta={MonteCarloGraph._beta} (guard distance score)\n"
        stats += f"delta={MonteCarloGraph._delta} (distance to visible area score)\n"
        stats += f"theta={MonteCarloGraph._theta} (distance to shadow score)\n"
        stats += f"gamma={MonteCarloGraph._gamma} (kernel distance score)\n"
        stats += f"tau={MonteCarloGraph._tau} (kernel distance-decay rate)\n"
        stats += f"top_k={MonteCarloGraph._top_k} (num of kernels evaluated for score)\n"
        stats += f"eta={MonteCarloGraph._eta} (depth bonus)\n"
        stats += f"lambda={MonteCarloGraph._lambda} (weight of rollout value in heuristic)\n"
        stats += f"phi={MonteCarloGraph._phi} (rollout depth)\n"  
        stats += f"using rollout ratio={MonteCarloGraph._rollout_ratio}\n"  
        stats += f"visible node penalty = {MonteCarloGraph._visible_node_penalty}\n"
        stats += f"using brute force moves = {MonteCarloGraph._brute_force}\n"   

        stats += f"Total nodes in tree: {self._num_nodes}, with max depth {self._depth}\n"
    ...
    class Node:
        def __init__(self, tree : MonteCarloGraph, loc : Tuple[float, float], depth : int):
            self._tree = tree
            self._loc = loc
            self._depth = depth
            ...
            self._incoming_edges: List[MonteCarloGraph.Edge] = []
            self._outgoing_edges: List[MonteCarloGraph.Edge] = []
        ...
        def get_score(self):
            if (self._score is None):
                if self._map.is_visible(self._loc, self._depth):
                    ...
                    shadow_score = MonteCarloGraph._theta * math.exp(-MonteCarloGraph._tau * d)
                    shadow_score += MonteCarloGraph._visible_node_penalty
                else:
                    vis_area = self._map.get_visibility_polygon(self._depth)
                    d = Point(self._loc).distance(vis_area)
                    shadow_val = 1.0 - math.exp(-MonteCarloGraph._tau * d)
                    shadow_score = MonteCarloGraph._delta * shadow_val
                    #print(f"Shadow distance {d} gave score {shadow_val}, weighted {shadow_score}")
                ...
                for pos in guard_positions:
                    ...
                    shortest_paths += path_length
                #guard_score = (shortest_path/(1.0+shortest_path))
                guard_score = 1.0 - math.exp(-MonteCarloGraph._tau * (shortest_paths/len(guard_positions)))
                ...
                guard_distance_score = MonteCarloGraph._beta * (guard_score)
                #print(f"Average guard distance {shortest_paths} gave score {guard_score}, weighted {guard_distance_score}")

                # find distance from the player to the 3 deepest kernels
                kernels = self._map.get_kernels(self._depth, True)
                top_k = kernels[:MonteCarloGraph._top_k] if len(kernels) >= MonteCarloGraph._top_k else kernels[:]
                path_dists = []
                weights = []
                for ker in top_k:
                    _, path_length = self._map.get_shortest_path(self._loc, ker.get_coords())
                    #print(f"Score planning to kernel at {ker.get_coords()} w depth {ker.get_depth()} at timestep {self._depth}")
                ...
                score = math.exp(-MonteCarloGraph._tau * avg_dist)
                #score = (avg_dist/(1.0+avg_dist))
                
                #print(f"Normalized path distance: {norm}, for distances {dist for dist in path_dists}")
                kernel_score = MonteCarloGraph._gamma * score
                #print(f"Average path distance {avg_dist} gave score {score}, weighted {kernel_score}")
                ...
                # depth bonus 
                depth_bonus = MonteCarloGraph._eta * (self._depth / self._tree._max_depth) ** 2

                # rollout score
                rollout_score = MonteCarloGraph._lambda * self._tree.rollout(self)
        ...
        def get_potential_moves(self) -> List[Tuple[int|float, int|float]]:
            if self._potential_moves is None:
                ...
                if (MonteCarloGraph._brute_force):
                    moves.extend(self.get_brute_force_moves())

                random.shuffle(moves)
                # Fallback if no moves, just stay still
                if len(moves) == 0:
                    moves = [self._loc]
    ...
    class Edge: 
        def __init__(self, parent: MonteCarloGraph.Node, child: MonteCarloGraph.Node, path: LineString):
            self._parent = parent
            self._child = child
            self._path = path
        ...
        def ucb_score(self):
            if self._visits == 0:
                ...
                exploration = MonteCarloGraph._c * math.sqrt(
